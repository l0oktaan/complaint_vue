                ...
                <span class="h1">ขั้นตอนการร้องเรียนเรื่องทุจริต</span>
                    ...
                    <v-stepper v-model="e1" class="step">
                        <v-stepper-header class="stepper-header mt-3">
                                ...
                                <!-- <p>ข้อตกลงเเละเงื่อนไขการร้องเรียน</p> -->
                            ...
                            <v-stepper-content step="4">
                                ...
                                <v-row justify="space-between">
                                    <v-col v-if="user.item" cols="12" md="6" class="text-left">
                                        ...
                                        <!-- <p>ที่อยู่ : {{ user.item.address }} {{ user.subdistrict_value }} {{ user.district_value }} {{ user.province_value }} {{ user.postcode }}</p> -->
                                    </v-col>
...
import StepTree from '@/components/step/stepTree.vue'
  ...
  components: { stepOne, StepTwo, StepTree },
    ...
    // mounted() {
    //     // Get the active tab index from local storage
    //     const activeTabIndex = localStorage.getItem('activeTabIndex');
    //     if (activeTabIndex) {
    //         // Set the active tab to the stored index
    //         this.e1 = parseInt(activeTabIndex);
    //     }
    // },
    methods: {
         // checkStep(index) {
        ...
        //         }
        //     }
        //     localStorage.setItem('activeTabIndex', this.e1.toString());
        ...
        async submit(){
           try {
                let fd = await {
                    ...
                    // "address"               : `${this.user.address + ' ' + this.user.subdistrict_value + ' ' + this.user.district_value + ' ' + this.user.province_value + ' ' + this.user.postcode}`,
                    // "address"               : `${this.user.address 
                    //                             + (this.user.province_value == 'กรุงเทพมหานคร'?' แขวง':' ตำบล') + this.user.subdistrict_value 
                    //                             +(this.user.province_value == 'กรุงเทพมหานคร'?' เขต':' อำเภอ') + this.user.district_value
                    //                             +(this.user.province_value == 'กรุงเทพมหานคร'?' ':' จังหวัด') + this.user.province_value
                    //                             +' ' +this.user.postcode}`,   
