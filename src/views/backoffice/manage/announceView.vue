        ...
        <v-card-text>
          ...
          <v-overlay v-if="loading"
            ...
            <v-form ref="formAnnounce" v-model="valid" lazy-validation>
              ...
              <!-- <InputFiles ref="announce_files" :file="dataDailogAnnounce.file"/> -->

              <!-- <pre>{{ dataDailogAnnounce }}</pre>
              ...
              <v-file-input 
                ...
                <div v-else class="text-left">
                  <v-chip 
                    ...
                    closable
                  >
                    <v-icon v-if="dataDailogAnnounce.file_type == 'application/pdf'" @click="showFile(dataDailogAnnounce.file_type, dataDailogAnnounce.file_name, 'UrlFilesAnnounce')">mdi-file</v-icon>
              ...
              <!-- <div v-else class="announce_files text-left">
                <div v-if="!checkRemoveFile" >
                  <v-chip 
                    class="ma-2"
                    closable
                  >
              ...
              <!-- <div class="announce_files text-left">
                <v-chip 
                  ...
                  closable
                >
                  <v-icon v-if="dataFile.file_type == 'application/pdf'" @click="showFile(dataFile.file_type, dataFile.file_name, 'UrlFilesAnnounce')">mdi-file</v-icon>
                  <v-icon v-else @click="showFile(dataFile.file_type, dataFile.file_name, 'UrlFilesAnnounce')">mdi-image</v-icon>
                  &nbsp;{{dataFile.file_original}}
                  &nbsp; <v-icon @click="removeFile(dataFile)">mdi-close</v-icon>
                </v-chip>
              </div> -->
            
                      
            <!-- <DatePickers ref="test" @change_date="changeStartDate" @change_end_date="changeEndDate" :show_date="dataDate"/> -->
              <v-row>
                <v-col>
...
export default {
  components: { breadcrumbsView,  LoaderView},
  data: () => ({
  ...
  methods:{
    getEndThaiDate(){
      if (this.start_date || this.end_date){
          ...
          // console.log(this.dataDailogAnnounce.start_date);
    ...
    // async getAnnounceFiles(id){
    //   let path                = await `/api/backoffice/get/announceFiles`
    //   let response            = await axios.get(`${path}/`+ id)
    //   this.dataFiles          = await response.data.dataFiles
    //   await setTimeout(() => (this.$refs.loader.overlay = false), 300);
    // },
        ...
        try {
          ...
          if(this.titleAnnounce === -1){
            ...
            fd = await {
              ...
              // "start_date"              : this.dataDailogAnnounce.start_date,
              // "end_date"                : this.dataDailogAnnounce.end_date,
            ...
            // if(this.dataDailogAnnounce.file){

            //   const file = this.dataDailogAnnounce.file

            //   const arr_file = await file.type.split("/")

            //   if(file.type === 'image/jpeg' || file.type === 'image/jpg' || file.type === 'image/png'){

            //     fileName = await 'imgAnnounce' +  this.announceId + '_1' + '.' +arr_file[1] 

            //   }else if(file.type === 'application/pdf'){

            //     fileName = await 'pdfAnnounce' +  this.announceId + '_1' + '.' +arr_file[1] 
            //   }

            // }

            // let data = await this.dataDailogAnnounce.start_date
          ...
          if(response && this.dataDailogAnnounce.file !== undefined){



            // console.log(this.dataDailogAnnounce.file_name);

            

           

            // for (let index = 0; index < this.$refs.announce_files.files.length; index++) {
              
            //   let number = await index + 1

            //   await formData.append('files', this.$refs.announce_files.files[index])
            // const arr_data_file = await await this.dataDailogAnnounce.file_name.split(/(\d+)/)
            // console.log(this.dataDailogAnnounce.file_name);
            // console.log(arr_data_file);

              // await this.saveAnnounceFiles(response.data.announce_id, response.data.file_name, this.dataDailogAnnounce.file, this.titleAnnounce)
            // }

            // if(this.dataDailogAnnounce.file !== null){
    ...
    // async saveAnnounceFiles (announce_id, file_name, file, type_announce){

    // console.log(type_announce);
    ...
    //     file_name = await 'imgAnnounce' +  announce_id + '_1' + '.' +arr_file[1] 
    //     // file_name = await 'imgAnnounce' +  announce_id + '_' + number + '.' +arr_file[1] 

    //   }else if(file.type === 'application/pdf'){

    //     file_name = await 'pdfAnnounce' +  announce_id + '_1' + '.' +arr_file[1] 
    //     // file_name = await 'pdfAnnounce' +  announce_id + '_' + number + '.' +arr_file[1] 
    //   }
    ...
    //   let path_api = await `/api/backoffice/update/announceFile`

    //   await axios.post(`${path_api}`, fd_upload )

    //   const formData = await new FormData(); 

    //   await formData.append('id', announce_id);
    ...
    async editAnnounce(value){
      ...
      // this.announceFileName        = await value.file_name;
      // this.announceFileType        = await value.file_type;
      // this.announceFileOrifinal    = await value.file_original;
      ...
      // this.dataDailogAnnounce.start_date = dStart.toLocaleDateString('th-TH', { day: 'numeric', month: 'long', year: 'numeric' });

      // var dEnd = new Date(value.end_date);
      // this.dataDailogAnnounce.end_date = dEnd.toLocaleDateString('th-TH', { day: 'numeric', month: 'long', year: 'numeric' });

      // this.dataDailogAnnounce.start_date = await moment(value.start_date).format("DD MMMM YYYY");
      // this.dataDailogAnnounce.end_date = await moment(value.end_date).format("YYYY-MM-DD");
      ...
      // console.log(this.dataDailogAnnounce);

      // await this.getAnnounceFiles(value.id)
        ...
        await Swal.fire({
          ...
          cancelButtonText: 'ยกเลิก',
            ...
            if (result.isConfirmed) {              
                ...
                // await this.getAnnounce()
                // await this.getAnnounceFiles(v.announce_id)
            }
        })
    ...
    //   this.dataDailogAnnounce.start_date = date
    // },
    // changeEndDate(date){
    //   this.dataDailogAnnounce.end_date = date
    // },
    getThaiDate(value){
      if (value){
        var d = new Date(value);
